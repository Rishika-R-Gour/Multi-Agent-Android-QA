...
class AndroidEnv:
    ...
    def __init__(self, task_name="settings_wifi", enable_real_device=False):
        ...
        if self.enable_real_device and android_world_controller:
            ...
            adb_available = self._check_adb_connection()
            if adb_available:
                # Initialize real AndroidWorld controller with proper task config
                try:
                    # For real device mode, we need to create a proper AndroidEnv instance
                    # instead of passing task_config which isn't expected
                    self.controller = android_world_controller.AndroidWorldController(
                        env=DummyEnv(task_name),  # Use DummyEnv as base, AndroidWorld will enhance it
                        install_a11y_forwarding_app=True
                    )
                    self.logger.info(f"Real device mode initialized for task: {task_name}")
                except Exception as e:
                    self.logger.error(f"Failed to initialize real device mode: {e}")
                    self.logger.info("Falling back to simulation mode")
                    self.controller = None
                    self.enable_real_device = False
            else:
                self.logger.warning("ADB connection not available. Falling back to simulation mode.")
                self.controller = None
                self.enable_real_device = False
        else:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if connected_devices:
                    ...
                    self.logger.warning("ADB is available but no devices connected")
                    return False
            else:
                self.logger.warning("ADB command failed")
                return False
        except (subprocess.TimeoutExpired, FileNotFoundError) as e:
            self.logger.warning(f"ADB not available: {e}")
            return False
    
    def _get_task_config(self, task_name: str) -> Dict[str, Any]:
        """Get task configuration for AndroidWorld."""
        # Basic task configurations for different task types
        task_configs = {
            'settings_wifi': {
                'task_id': 'settings_wifi',
            ...
            'clock_alarm': {
                ...
                'activity': 'com.google.android.deskclock.DeskClock',
                'description': 'Test alarm creation and management'
            },
            'email_search': {
                'task_id': 'email_search',
        ...
        return task_configs.get(task_name, {
            'task_id': task_name,
            'app_package': 'com.android.settings',  # Default to settings
            'activity': 'com.android.settings.Settings',
            'description': f'Test {task_name} functionality'
        })
